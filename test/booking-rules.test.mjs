...
test('party-size duration follows the active Standard mode', () => {
  assert.deepEqual([1, 2, 3, 4, 5, 6, 7, 8, 9, 20].map(durationMinutes), [90, 90, 120, 120, 150, 150, 180, 180, 210, 210]);
  assert.throws(() => durationMinutes(21), RangeError);
});
...
test('public slots are half-hourly and keep the shisha period outdoors', () => {
  assert.deepEqual(slotsForService(SERVICES[0]), ['13:00', '13:30', '14:00', '14:30', '15:00']);
