// Booking rules observed in Du Liban's SevenRooms audit (19 September 2026).
...
export const AREAS = Object.freeze({
  ...
  chillout: { label: 'Chillout', bookable: true },
...
export function durationMinutes(partySize) {
  if (!Number.isInteger(partySize) || partySize < 1 || partySize > MAX_PARTY_SIZE) {
    throw new RangeError('La reserva debe ser de 1 a 20 personas');
  }
...
export function slotsForService(service) {
  const slots = [];
  for (let t = minutes(service.start); t <= minutes(service.end); t += 30) {
